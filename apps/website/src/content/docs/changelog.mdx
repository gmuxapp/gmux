...
`gmux -- <cmd>` to run, `gmux open` for the UI, and `ls` / `attach` / `tail` / `send` / `wait` / `kill` / `edit` as verbs. The old flags and bare-command shorthand are gone, but every removed form prints its exact replacement. Power users: `alias gm='gmux --'`.

### Multi-machine, locked down

Being on your tailnet now only lets a machine *reach* a host; the host's token authorizes it. Autodiscovery is replaced by explicit, token-authenticated hosts (`gmux auth` → **Settings → Hosts → Connect to host**), and cookie-authed requests are same-origin enforced. Upgrading carries existing hosts forward as "Auth needed" so nothing silently disappears.

### A rebuilt web app

- **Home is an activity dashboard** — Waiting and Active first, then recency buckets; project and host management moved into a proper **Settings** modal.
...
Dead sessions survive daemon restarts and stay resumable, with scrollback as a bounded cache so disk can't grow forever. Dismiss is permanent, and alive and resumable sessions now share the same terminal chrome.
...
Managed editor tabs that work as `$EDITOR` — `git commit` inside a session opens a real gmux tab and blocks until you're done.
