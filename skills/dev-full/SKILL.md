...
Builds `gmuxd-dev` from source, runs it on `:22226`, runs vite on `:5173`, and
...
This blocks — keep it running in a background session. The app is at **http://localhost:5173**; the dev daemon API is at `:22226`.

> `:22226` is the dev daemon. `:8790` is production. They are intentionally far apart.
...
> navigate via `:5173`, not `:22226` — the auth cookie is origin-scoped.
