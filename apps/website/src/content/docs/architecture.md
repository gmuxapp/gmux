...
| `GET /v1/health` | Daemon health, version, launchers, peer status, plus startup progress: `conversation_index` (background conversation indexing) and `session_recovery` (runner rediscovery after a restart) |
