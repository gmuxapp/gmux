...
Dead sessions survive daemon restarts because they are rows in the SQLite database. When a session exits, its state is committed to `state.db`; on startup gmuxd serves rows from SQLite right away while it rediscovers surviving runners and merges their live state, reporting that convergence as `session_recovery` (with expected and recovered counts) in `GET /v1/health`. Its `status` is a monotone three-state machine: `recovering` while the bounded convergence window is open, `degraded` if that pass swept sessions dead it had abandoned without ever getting an answer about them (recovery is finished but not complete — treat it as terminal and unhealthy rather than waiting; only one thing promotes it to `ready`, the abandoned runner registering *alive* again via the periodic discovery pass, so a runner that answers dead — or never answers again — leaves the daemon `degraded` until restart), and `ready` when recovery is complete or definitively finished — including when a candidate is provably gone. A daemon with no previously-alive rows reports `ready 0/0` immediately. The status is decided when the convergence window closes and only ever promoted; the `expected`/`recovered` counts are live and informational, so an ordinary session exit lowers `recovered` without changing the status. Gate on the status, never on the counts or on "not `recovering`". There is no separate sweep or JSON file to synchronize.
