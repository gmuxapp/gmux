...
const (
	...
	// tokenAttempts is the number of times readToken is retried for a
	// container whose gmuxd is still starting up (auth-token file not
	// yet written). Retries run in parallel across candidate containers.
	tokenAttempts = 3
	// tokenRetryDelay is the delay between readToken attempts.
	tokenRetryDelay = 1 * time.Second
	gmuxdPort       = 8790
)
...
// Slow operations (RemovePeer, readToken) run outside the lock so tests
// calling Tracked() don't block.
func (w *Watcher) scan(ctx context.Context) {
	...
	// Compute removals and candidates under the lock (fast).
	w.mu.Lock()
	...
	// Execute removals outside the lock (RemovePeer blocks until the
	// peer goroutine finishes, which can take seconds).
	for _, r := range removals {
		log.Printf("devcontainers: container %s stopped, removing peer %s", short(r.id), r.name)
		w.mgr.RemovePeer(r.name)
	}

	// Read tokens concurrently with bounded retries (gmuxd inside the
	// container may still be starting up and haven't written the token).
	type registration struct {
		c     container
		token string
	}
	results := make(chan registration, len(candidates))
	var wg sync.WaitGroup
	for _, c := range candidates {
		wg.Add(1)
		go func(c container) {
			defer wg.Done()
			token, err := w.readTokenWithRetry(ctx, c.ID)
			if err != nil {
				log.Printf("devcontainers: container %s not ready (%v), will retry on next event", short(c.ID), err)
				return
			}
			results <- registration{c, token}
		}(c)
	}
	wg.Wait()
	close(results)

	// Register peers under the lock.
	w.mu.Lock()
	defer w.mu.Unlock()
	for r := range results {
		...
		w.mgr.AddPeer(cfg)
		w.tracked[r.c.ID] = name
	}
}

// readTokenWithRetry calls docker.readToken up to tokenAttempts times
// with tokenRetryDelay between attempts, so we catch gmuxd instances
// that took longer than our first attempt to write their auth token.
func (w *Watcher) readTokenWithRetry(ctx context.Context, id string) (string, error) {
	var lastErr error
	for attempt := 0; attempt < w.tokenAttempts; attempt++ {
		if attempt > 0 {
			select {
			case <-ctx.Done():
				return "", ctx.Err()
			case <-time.After(w.tokenRetryDelay):
			}
		}
