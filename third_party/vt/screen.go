...
type Screen struct {
	...
	scroll uv.Rectangle
	// wrapped records whether each visual row continues into the next row.
	// It is kept here because ultraviolet.RenderBuffer has no row metadata.
	wrapped []bool
...
// Wrapped reports whether row y ended because a pending autowrap was consumed.
	...
	if s.scrollback != nil {
		...
		for y := 0; y < s.buf.Height(); y++ {
			...
			if line != nil && !s.isLineEmpty(line) {
				s.scrollback.PushWrapped(line, s.Wrapped(y))
			}
		}
	}
	s.Clear()
...
func (s *Screen) DeleteLine(n int) bool {
	...
	if s.scrollback != nil && y == scroll.Min.Y &&
		...
		// Save lines that will be deleted, including wrap provenance.
		linesToSave := min(n, scroll.Max.Y-y)
		s.scrollback.pushN(s, y, linesToSave)
	}
...
// insertWrappedLines mirrors RenderBuffer.InsertLineArea row movement.
func (s *Screen) insertWrappedLines(y, n int, area uv.Rectangle) {
	...
	if area.Min.X != 0 || area.Max.X != s.Width() {
		for row := y; row < area.Max.Y; row++ {
			s.setWrapped(row, false)
		}
		return
	}
...
// deleteWrappedLines mirrors RenderBuffer.DeleteLineArea row movement.
